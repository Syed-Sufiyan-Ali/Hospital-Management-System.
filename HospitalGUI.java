...
abstract class BasePanel extends JPanel {
    ...
    BasePanel() { setBackground(new Color(245, 247, 250)); }
...
class AddPanel extends BasePanel {
    ...
    AddPanel() {
        ...
        for (int i = 0; i < labels.length; i++) {
            ...
            form.add(lbl); form.add(fields[i]);
        }
        status = new JLabel(" ");
        status.setFont(new Font("Segoe UI", Font.ITALIC, 12));
    ...
    private void addPatient() {
        loadPatients();
        if (totalPatients >= 10) { setStatus("Hospital full! Max 10 patients.", new Color(192, 57, 43)); return; }
        ...
        for (int i = 0; i < totalPatients; i++) {
            if (patients[i][3].equals(id)) { setStatus("Patient ID already exists!", new Color(192, 57, 43)); return; }
        }
    ...
    private void setStatus(String msg, Color color) { status.setText("  " + msg); status.setForeground(color); }
    public void refresh() { }
}
...
class BillPanel extends BasePanel {
    ...
    BillPanel() {
        ...
        heading.setFont(new Font("Segoe UI", Font.BOLD, 16));
        heading.setForeground(new Color(44, 62, 80));
        JTextField fId = new JTextField(), fDays = new JTextField();
        ...
        form.add(l1); form.add(fId); form.add(l2); form.add(fDays);
        ...
        btnBill.addActionListener(e -> {
            ...
            try {
                ...
                if (name == null) { resultLabel.setText("  Patient ID not found!"); resultLabel.setForeground(new Color(192,57,43)); return; }
                resultLabel.setText("<html><b>Patient:</b> " + name + " &nbsp;|&nbsp; <b>Days:</b> " + days + " &nbsp;|&nbsp; <b>Total Bill:</b> Rs. " + (days * 1000) + "</html>");
                resultLabel.setForeground(new Color(39, 174, 96));
            } catch (NumberFormatException ex) {
                resultLabel.setText("  Enter valid number for days!"); resultLabel.setForeground(new Color(192,57,43));
            }
        });
        JPanel top = new JPanel(new BorderLayout());
        top.setOpaque(false);
...
public class HospitalGUI extends JFrame {
    ...
    HospitalGUI() {
        setTitle("Sufiyan Health Clinic - Management System");
        ...
        JLabel appName = new JLabel("<html><center>SUFIYAN<br>HEALTH CLINIC</center></html>", SwingConstants.CENTER);
        ...
        cardPanel.add(new ViewPanel(),   "view");
        cardPanel.add(new AddPanel(),    "add");
        cardPanel.add(new SearchPanel(), "search");
        cardPanel.add(new BillPanel(),   "bill");
        cardPanel.add(new DeletePanel(), "delete");
        ...
        for (String[] item : navItems) {
            StyledButton btn = new StyledButton(item[0], Color.decode(item[2]));
        ...
        // Developer credit
        JLabel devLabel = new JLabel("Dev: Sufiyan", SwingConstants.CENTER);
        devLabel.setFont(new Font("Segoe UI", Font.ITALIC, 11));
        devLabel.setForeground(new Color(149, 165, 166));
        devLabel.setAlignmentX(Component.CENTER_ALIGNMENT);
        devLabel.setBorder(BorderFactory.createEmptyBorder(0, 0, 6, 0));
        ...
        btnExit.addActionListener(e -> {
            if (JOptionPane.showConfirmDialog(this, "Exit?", "Exit", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION)
                System.exit(0);
        });
        sidebar.add(btnExit);
